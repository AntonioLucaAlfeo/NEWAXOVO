...
from src.TAWAX.OVOBaseLayer import OVOBaseLayer
from src.TAWAX.OVABaseLayer import OVABaseLayer
from src.TAWAX.TAWAXSuperClassifierLayer import TAWAXSuperClassifierLayer
...
class TAWAX:
    ...
    def __init__(self, n_classes, subject_id, models_path="models/", results_path="results/",
                 base_layer_mode="1vs1", samples_balancing="none"):

        """
          This function initialises the TAWAX object using the given parameters. This will be the skeleton of
          TAWAXOVO/TAWAXOVA architecture.
        ...
        :param base_layer_mode: one between '1vs1' and '1vsALL'. If '1vs1', base classifiers are trained in a 1 vs 1
            manner (TAWAXOVO). If '1vsALL', base classifiers are trained in a 1 vs ALL manner (TAWAXOVA).
            Default '1vs1'.
    ...
    def fit(self, x, y):
        ...
        self.base_classifiers_layer.fit(x, y)
    ...
    def predict(self, x):
        ...
        predictions = np.argmax(self.base_classifiers_layer.predict(x), axis=1) + 1
    ...
    def processing_history_predict(self, x, y, ids):
        ...
        history_filename = self.base_path + "history_predictions/tawax/processing_history_tawax_resampling_%s_%s.csv" \
                           % (self.samples_balancing, time.strftime("%Y%m%d-%H%M%S"))
        DatasetUtils.add_column_to_csv(filepath=history_filename,
                                       columns_values=np.repeat(self.subject_id, x.shape[0]),
                                       columns_names=np.array(["SUBJECT"]))
        DatasetUtils.add_column_to_csv(filepath=history_filename, columns_values=ids, columns_names=np.array(["ID"]))
        base_layer_predictions = self.base_classifiers_layer.processing_history_predict(x, filename=history_filename)
        DatasetUtils.add_column_to_csv(filepath=history_filename, columns_values=base_layer_predictions,
                                       columns_names=np.array(["sum(p(LH))", "sum(p(RH))", "sum(p(T))", "sum(p(F))"]))

        competence_scores_diff = np.zeros(x.shape[0])
        competence_scores_ratio = np.zeros(x.shape[0])

        for index in np.arange(x.shape[0]):
            c = base_layer_predictions[index, y[index] - 1]
            all = np.sum(base_layer_predictions[index, :])
            others = all - c
            competence_scores_diff[index] = c - others
            competence_scores_ratio[index] = c / all

        final_predictions = np.argmax(base_layer_predictions, axis=1) + 1
        DatasetUtils.add_column_to_csv(filepath=history_filename, columns_values=final_predictions,
                                       columns_names=np.array(["PREDICTION"]))
        DatasetUtils.add_column_to_csv(filepath=history_filename, columns_values=y, columns_names=np.array(["LABEL"]))
        DatasetUtils.add_column_to_csv(filepath=history_filename, columns_values=competence_scores_diff,
                                       columns_names=np.array(["COMPETENCE REINFORCEMENT SCORE DIFF"]))
        DatasetUtils.add_column_to_csv(filepath=history_filename, columns_values=competence_scores_ratio,
                                       columns_names=np.array(["COMPETENCE REINFORCEMENT SCORE RATIO"]))

        with open(self.base_path + "history_predictions/tawax/tawaxovo_all_diff.csv", "a") as f:

            np.savetxt(f, competence_scores_diff, delimiter=",")

        with open(self.base_path + "history_predictions/tawax/tawaxovo_all_ratio.csv", "a") as f:
    ...
    def __explain_majority_class(self, x, ids, q, k, p):
        ...
        if self.base_layer_mode == "1vs1":
            ...
            # compute TAWAXOVO predictions for each sample
            ...
            output_filename = self.base_path + "explanations_tawaxovo_mode_majority_subject_%d_%s.txt" % (self.subject_id,
    ...
    def __explain_all_classes(self, x, ids, q, k, p):
        ...
        if self.base_layer_mode == "1vs1":
            ...
            # name of the file in which explanations are stored
            output_filename = self.base_path + "explanations_tawaxovo_mode_allclasses_subject_%d_%s.txt" % (self.subject_id,
                                                                                           time.strftime(
                                                                                               "%Y%m%d-%H%M%S"))
    ...
    def __explain_error(self, x, ids, y, q, k, p):
        ...
        if self.base_layer_mode == "1vs1":
            ...
            # name of the file in which explanations are stored
            output_filename = self.base_path + "explanations_tawaxovo_mode_error_subject_%d_%s.txt" % (self.subject_id,
                                                                                      time.strftime("%Y%m%d-%H%M%S"))
    ...
    def __explain_ids(self, x, ids, contrastive_samples, contrastive_classes, k, p):
        ...
        if self.base_layer_mode == "1vs1":
            ...
            # name of the file in which explanations are stored
            output_filename = self.base_path + "explanations_tawaxovo_mode_ids_subject_%d_%s.txt" % (self.subject_id,
                                                                                    time.strftime("%Y%m%d-%H%M%S"))
